...
function syncTreeGetServerValue(syncTree, query) {
    ...
    const serverCacheNode = serverCacheComplete
        ...
        : null;
    const writesCache = syncTree.pendingWriteTree_.childWrites(query.path);
    ...
    if (tag) {
        ...
        const queriesToStop = subtree.fold((relativePath, maybeChildSyncPoint, childMap) => {
            if (!pathIsEmpty(relativePath) &&
                ...
                let queries = [];
                if (maybeChildSyncPoint) {
                    queries = queries.concat(syncPointGetQueryViews(maybeChildSyncPoint).map(view => view.getQuery()));
                }
...
function repoStartTransaction(repo, path, transactionUpdate, onComplete, applyLocally) {
    ...
    if (newVal === undefined) {
        ...
        if (transaction.onComplete) {
            ...
            const snapshot = new DataSnapshot(transaction.currentInputSnapshot, new Reference(repo, transaction.path), PRIORITY_INDEX);
