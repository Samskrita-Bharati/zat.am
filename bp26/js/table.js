...
function addMonthMenu() {
  ...
  $(".month-table").each(function (_, table) {
    ...
    $("#cal-controls").append(
      '<a class="month-menuitem" data-target="' +
        month +
        '" href="#' +
        month +
        '">' +
...
function appendEvent(event) {
  ...
  if (event.weekday == "s") {
    if (o == "aam") {
      ...
      var eventElement = $(
        '<div class="event pastd oks"><a target="sc23" href="' +
          hrefSpecial +
          '">' +
          event.name +
          " - " +
    ...
    if (eventStartDate < today) {
      ...
      var eventElement = $(
        '<div class="event pastd"><a target="sc23" href="' +
          hrefPast +
          '">' +
          event.name +
          " - " +
      ...
      var eventElement = $(
        '<div class="event">' + event.name + " - " + event.time + "</div>",
      );
    }
  }
  // Handle multi-days
  if (eventEndDate.getDate()) {
    ...
    var spacerNumber = $("#" + formattedDate(eventStartDate)).find(
      ".event",
    ).length;
    ...
    while (eventEndDate > date) {
      ...
      dateElement
        .removeClass("no-event")
        .append(
          '<div class="event multi-days following-days" title="' +
            event.name +
...
function generateMonthTable(date) {
  ...
  var monthTable = $(
    '<table cellspacing=0 class="month-table" data-month="' +
      date.getFullYear() +
      "-" +
      eventMonthName +
      '" id="month-' +
  ...
  loopForTimes(numberOfDays, function (daynumber) {
    ...
    getFirstAvailableRow(monthTableBody).append(
      '<td class="no-event ' +
        pastClass +
        '" id=' +
        id +
