...
; ~function (global) {
    ...
    var HERO_WIDTH = 26;
    var ARROW_HEIGHT = 15, ARROW_WIDTH = 5; 

    var FLOOR_VELOCITY_BASE = -0.10; // Floor rising speed
    var GRAVITY_ACC = 0.0015; // Gravity acceleration
    var SPRINGING_VELOCITY = -0.5; // Initial velocity when leaving the spring
    var SPRING_TIME = 100; // Spring compression time
    var FAKE_FLOOR_TIME = 300, FAKE_FLOOR_TIME2 = 600; // Fake floor stay time, fake floor rotation time
    var ROLLING_VELOCITY = 0.1; // Conveyor belt speed
    var CONTROL_VELOCITY = 0.2; // Left and right control speed
    ...
    function drawAll(context, time) {
        ...
        if (!isRunning) {
            ...
            if (isCooldownTime) {
                ...
                context.lineWidth = 6;
                context.strokeText('समाप्तम् ।', 0, 10);
                context.fillStyle = '#000';
                context.fillText('समाप्तम् ।', 0, 10);
            } else {
                ...
                context.textAlign = 'center';
                context.fillText(judge() ? 'पुनः क्रीडतु' : 'अनुवर्तते', 0, 10);
            }
            context.restore();
        }
        ...
        if (topBarChange) {
            ...
            context.fillText('जीवनम्‌ ' + 'oooooooooo----------'.substr(10 - hero.life, 10) + '   प्राप्ताङ्काः  ' + Sanscript.t(String(score),'iast', 'devanagari'), 10, 24);
            context.restore();
        }
